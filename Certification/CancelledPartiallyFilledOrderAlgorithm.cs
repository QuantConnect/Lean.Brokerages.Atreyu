...
{
    /// <summary>
    /// E3 - A cancel request is sent against a partially filled order
    /// order eventually cancelled
    /// </summary>
    /// <meta name="tag" content="using data" />
    /// <meta name="tag" content="using quantconnect" />
    /// <meta name="tag" content="trading and orders" />
    public class CancelledPartiallyFilledOrderAlgorithm : BaseAtreyuCertificationTestAlgorithm
    {
        protected override string TestCode { get; } = "E3";
        ...
        public override void OnData(Slice data)
        {
            foreach (var bar in data.Bars)
            {
                ...
                {
                    if (symbol.Executions?.Any() == true && !_cancelPending && symbol.Ticket.QuantityFilled == 50)
                    {
        ...
        public override void OnEndOfAlgorithm()
        {
            ...
            {
                if (exec.Ticket.QuantityFilled != 50)
                {
                    throw new Exception($"Filled Quantity {exec.Ticket.QuantityFilled}; expected 50.");
                }
            }
        }
    }
