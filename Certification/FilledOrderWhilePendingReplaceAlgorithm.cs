...
{
    /// <summary>
    /// E1 - An order gets filled while still pending cancel
    /// resulting to cancel rejection (too late to cancel)
    /// </summary>
    /// <meta name="tag" content="using data" />
    /// <meta name="tag" content="using quantconnect" />
    /// <meta name="tag" content="trading and orders" />
    public class FilledOrderWhilePendingReplaceAlgorithm : BaseAtreyuCertificationTestAlgorithm
    {
        protected override string TestCode { get; } = "E2a";
                ...
                if (Executions.TryGetValue(bar.Key.Value, out var symbol))
                {
                    if (symbol.Executions?.Any() == true && !_replacePending)
                    {
                        Transactions.UpdateOrder(new UpdateOrderRequest(
                            DateTime.UtcNow,
                            symbol.OrderId,
                            new UpdateOrderFields() { Quantity = 100 }));
