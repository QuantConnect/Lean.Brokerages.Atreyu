...
namespace QuantConnect.Atreyu.Certification
{
    /// <summary>
    /// D13
    /// One cancel/replace request is issued which is accepted -
    /// another one is issued which is also accepted
    /// </summary>
    /// <meta name="tag" content="using data" />
    /// <meta name="tag" content="using quantconnect" />
    /// <meta name="tag" content="trading and orders" />
    public class FilledOrderDoubleReplaceAcceptedAlgorithm : BaseAtreyuCertificationTestAlgorithm
    {
        protected override string TestCode { get; } = "D13";
            ...
            foreach (var bar in data.Bars)
            {
                ...
                if (Executions.TryGetValue(bar.Key.Value, out var symbol))
                {
                    if (symbol.Executions?.Any() == true)
                    {
                        ...
                        if (symbol.Ticket.QuantityFilled == 28 && !_replace2)
                        {
                            Transactions.UpdateOrder(new UpdateOrderRequest(
                                DateTime.UtcNow,
                                symbol.OrderId,
        ...
        public override void OnOrderSubmitted(OrderEvent orderEvent)
        {
            Executions[orderEvent.Symbol.Value].Executions = new Queue<ExecutionEvent>(new[]
            {
                ...
                new ExecutionEvent {Status = OrderStatus.UpdateSubmitted, FillQuantity = 10},
                ...
                new ExecutionEvent {Status = OrderStatus.PartiallyFilled},
                new ExecutionEvent {Status = OrderStatus.Filled, FillQuantity = 32}
            });
        }
