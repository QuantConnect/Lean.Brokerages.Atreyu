...
{
    /// <summary>
    /// D10 - Cancel/Replace request for part filled order
    /// The requested order qty exceeds the cum qty.
    /// Order is replaced then filled
    /// </summary>
    /// <meta name="tag" content="using data" />
    /// <meta name="tag" content="using quantconnect" />
    /// <meta name="tag" content="trading and orders" />
    public class PartFilledOrderReplaceExceedsCumQtyAlgorithm : BaseAtreyuCertificationTestAlgorithm
    {
        protected override string TestCode { get; } = "D10";
        ...
        public override void OnOrderSubmitted(OrderEvent orderEvent)
        {
            Executions[orderEvent.Symbol.Value].Executions = new Queue<ExecutionEvent>(new[]
            {
                ...
                new ExecutionEvent {Status = OrderStatus.PartiallyFilled, FillQuantity = 20},
                new ExecutionEvent {Status = OrderStatus.UpdateSubmitted, FillQuantity = 20},
                new ExecutionEvent {Status = OrderStatus.PartiallyFilled},
                new ExecutionEvent {Status = OrderStatus.Filled, FillQuantity = 20}
            });
        }
